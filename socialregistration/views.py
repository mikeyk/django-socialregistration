...
def setup(request, template='socialregistration/setup.html',
	...
	if not getattr(settings, 'SOCIAL_GENERATE_USERNAME', False):
		...
		del request.session['socialregistration_user']
		del request.session['socialregistration_profile']
		return HttpResponseRedirect(getattr(settings, 'LOGIN_REDIRECT_URL', s(request)))
...
def facebook_login(request, template='socialregistration/facebook.html',
	...
	if user is None:
		request.session['socialregistration_user'] = User(username="none_yet")
...
def facebook_connect(request, template='socialregistration/facebook.html',
	...
	profile = get_object(FacebookProfile, "uid = ", request.facebook.uid, "user = ", request.user)
	if not profile:
		profile = db_create(FacebookProfile, "uid = ", request.facebook.uid, "user =", request.user)
...
def openid_redirect(request):
	...
	client = OpenID(
		request,
		'http://%s%s' % (
			Site.objects.get_current().domain, # TODO don't use Site framework
